...
def remove_punct(text):
    ...
    string_name=text.translate(translator)
    return (string_name)
...
def menu(exits):
    ...
    while True:
        ...
        user_input = input()
        # Normalise the input
        normal = normalise_input(user_input)
        # Check if the input makes sense (is valid exit)
        valid = is_valid_exit(exits, normal)
            # If so, return the player's choice
        if valid == True:
            return user_input
        else:
            print("This exit is invalid, please enter another")
